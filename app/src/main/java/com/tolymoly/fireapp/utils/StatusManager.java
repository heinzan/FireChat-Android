...
public class StatusManager {
    ...
    public static void deleteStatus(final String statusId, final DeleteStatus onComplete) {
        FireConstants.getMyStatusRef().child(statusId).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
    ...
    public static void uploadStatus(final String filePath, final boolean isVideo, final UploadStatusCallback uploadStatusCallback) {
                        ...
                        uploadTask.getResult().getStorage().getDownloadUrl().addOnCompleteListener(new OnCompleteListener<Uri>() {
                            ...
                            public void onComplete(@NonNull Task<Uri> task) {
                                if (task.isSuccessful()) {
                                    ...
                                    FireConstants.getMyStatusRef().child(status.getStatusId()).updateChildren(status.toMap()).addOnCompleteListener(new OnCompleteListener() {
                                        @Override
